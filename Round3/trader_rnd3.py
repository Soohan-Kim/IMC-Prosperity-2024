...
class Trader:
    ...
    def order_basket(self, state):
        ...
        best_ask, best_bid, worst_ask, worst_bid, mid_price, weighted_bid, weighted_ask = {}, {}, {}, {}, {}, {}, {}
        ...
        for p in orders.keys():
            ...
            best_ask[p] = book_ask[0][0]
            best_bid[p] = book_bid[0][0]

            worst_ask[p] = book_ask[-1][0]
            worst_bid[p] = book_bid[-1][0]
            
            weighted_ask[p] = self.weighted_price(book_ask)
            weighted_bid[p] = self.weighted_price(book_bid)
            
            mid_price[p] = (best_ask[p] + best_bid[p])/2
        ...
        if spread > threshold:
            ...
            if vol > 0:
                orders['GIFT_BASKET'].append(Order('GIFT_BASKET', weighted_bid['GIFT_BASKET'], -vol//2))
                orders['GIFT_BASKET'].append(Order('GIFT_BASKET', worst_bid['GIFT_BASKET'], -vol//2))

        elif spread < -threshold:
            vol = self.POSITION_LIMIT['GIFT_BASKET'] - self.position['GIFT_BASKET']
            if vol > 0:
                orders['GIFT_BASKET'].append(Order('GIFT_BASKET', weighted_ask['GIFT_BASKET'], vol//2))
                orders['GIFT_BASKET'].append(Order('GIFT_BASKET', worst_ask['GIFT_BASKET'], vol//2))
