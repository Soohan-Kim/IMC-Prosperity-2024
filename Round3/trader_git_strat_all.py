...
class Trader:
    ...
    def order_basket(self, state):
        ...
        spread = mid_price['GIFT_BASKET'] - mid_price['CHOCOLATE']*4 - mid_price['STRAWBERRIES']*6 - mid_price['ROSES'] - self.spread_mean
        threshold = self.spread_std *0.5

        if spread > threshold:
            vol = self.position['GIFT_BASKET'] + self.POSITION_LIMIT['GIFT_BASKET']
            if vol > 0:
                # orders['GIFT_BASKET'].append(Order('GIFT_BASKET', wg_buy['GIFT_BASKET'], -vol/2))
            ...
            if vol > 0:
                # orders['GIFT_BASKET'].append(Order('GIFT_BASKET', wg_sell['GIFT_BASKET'], vol/2))
                orders['GIFT_BASKET'].append(Order('GIFT_BASKET', worst_sell['GIFT_BASKET'], vol))

        if spread > threshold:
            vol1 = self.position['CHOCOLATE'] + 58*4
            if vol1 > 0:
                orders['CHOCOLATE'].append(Order('CHOCOLATE', worst_buy['CHOCOLATE'], -vol1))
            
            vol2 = self.position['STRAWBERRIES'] + 58*6
            if vol2 > 0:
                orders['STRAWBERRIES'].append(Order('STRAWBERRIES', worst_buy['STRAWBERRIES'], -vol2))
            
            vol3 = self.position['ROSES'] + 58
            if vol3 > 0:
                orders['ROSES'].append(Order('ROSES', worst_buy['ROSES'], -vol3))
            
        elif spread < -threshold:
            vol1 = 58*4 - self.position['CHOCOLATE']
            if vol1 > 0:
                orders['CHOCOLATE'].append(Order('CHOCOLATE', worst_sell['CHOCOLATE'], vol1))
            
            vol2 = 58*6 - self.position['STRAWBERRIES']
            if vol2 > 0:
                orders['STRAWBERRIES'].append(Order('STRAWBERRIES', worst_sell['STRAWBERRIES'], vol2))
                
            vol3 = 58 - self.position['ROSES']
            if vol3 > 0:
                orders['ROSES'].append(Order('ROSES', worst_sell['ROSES'], vol3))
    ...
    def run(self, state: TradingState):
        ...
        orders = self.order_basket(state)
