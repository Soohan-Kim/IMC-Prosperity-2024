...
class Trader:
    ...
    def order_orchids(self, state):
        ...
        if self.prev_position != curr_position:
            self.conversions -= (curr_position - self.prev_position)
        ...
        for ask_price, vol in book_ask:
            if (ask_price < adj_bid_price):
                ...
                orders.append(Order(product, ask_price - 2, order_for))
        ...
        for bid_price, vol in book_bid:
            if (adj_ask_price < bid_price):
                ...
                orders.append(Order(product, bid_price + 2, order_for))
