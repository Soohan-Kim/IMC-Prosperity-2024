...
class Trader:
    ...
    def bs_formula(self, price, vol, r):
        def cdf(x):
            return 0.5 * (1 + math.erf(x / math.sqrt(2)))
        K = 10000
        d1 = (np.log(price/K) + (r+vol**2/2))/vol
        d2 = d1-vol
        return price*cdf(d1) - np.exp(-r)*K*cdf(d2)
        
    def order_coconut(self, state):
        ...
        ccn_ask, ccn_ask_amount = list(order_ccn.sell_orders.items())[0]
        ccn_bid, ccn_bid_amount = list(order_ccn.buy_orders.items())[0]
        ccn_mid = (ccn_ask + ccn_bid) / 2

        if order_cpn.sell_orders:
            cpn_ask, cpn_ask_amount = list(order_cpn.sell_orders.items())[0]
            self.prev_cpn_ask, self.prev_cpn_ask_amount = cpn_ask, cpn_ask_amount
        else:
            cpn_ask, cpn_ask_amount = self.prev_cpn_ask, self.prev_cpn_ask_amount
        if order_cpn.buy_orders:
            cpn_bid, cpn_bid_amount = list(order_cpn.buy_orders.items())[0]
            self.prev_cpn_bid, self.prev_cpn_bid_amount = cpn_bid, cpn_bid_amount
        else:
    ...
    def run(self, state: TradingState):
        ...
        coconut_orders = self.order_coconut(state)
